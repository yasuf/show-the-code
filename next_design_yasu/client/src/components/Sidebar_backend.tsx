    ...
    <div>
      <div className="p-0">
        ...
        <ul>
          {menuItems.map((item) => (
            <li key={item.name}>
                <div key={item.name} onClick={() => {setLanguage(item.language); setFile(item.filename)}} className={`flex items-center p-2 rounded-[2.85px] sidebarText h-[1.463rem] ${file==item.filename? 'bg-[#3154E0] text-[#3154E0]' : ''} ${language.pathname === item.name ? 'bg-blue-500' : 'text-gray-700 border-indigo-500'}`}>
