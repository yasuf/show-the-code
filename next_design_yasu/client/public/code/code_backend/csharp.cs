using AutomationWorks_Crafter.Interfaces;
using System;
...
namespace AutomationWorks_Crafter
{
    public class Detector : LocallyCached, IDetector
    {
        ...
        async public Task RegisterExchange()
        {
            ...
            IDetectorPlugin dp = Plugins.detectorPlugins.Where(x => x.Entity._id == _id).FirstOrDefault();
            if (dp == null) return;
            if (global.webSocketClient != null && global.webSocketClient.isConnected && global.commanderconfig != null && !string.IsNullOrEmpty(global.commanderconfig.version))
            {
                var ver = Version.Parse(global.commanderconfig.version);
